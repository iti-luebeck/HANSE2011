...
Module_Simulation::Module_Simulation(QString id, Module_UID *uid)
    : RobotModule(id)
{
    thread.start();
...
void Module_Simulation::refreshData()
{
    if (!getSettings().value("enabled").toBool())
        return;
...
void Module_Simulation::readPressure()
{
...
void Module_Simulation::readTemperature()
{
...
float Module_Simulation::getDepth()
{
    QMutexLocker l(&this->moduleMutex);
    return data["depth"].toFloat();
}

float Module_Simulation::getTemperature()
{
    QMutexLocker l(&this->moduleMutex);
...
bool Module_Simulation::readRegister(unsigned char reg, int size, char *ret_buf)
{
    unsigned char address = getSettings().value("i2cAddress").toInt();
